...
/*
unique_ptr<Module> Parser::parseModule() {
	LOG << "Step Module" << endl;
	LOG << tokenizer->lastToken()->getText() << endl;
	if (tokenizer->peek(TT_END))
		LOG << "End of the file!" << endl;
...
unique_ptr<Func> Parser::parseFunctionSignature() {
	LOG << "Step Function Signature" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;

	auto pos = tokenizer->previousPosition();
	if (tokenizer->peek(TT_IDENTIFIER)) {
		if (tokenizer->peek(TF_FUNCTION)) {
			logPos();
			ERROR << "Multiple declarations found of: " << tokenizer->gCurrentToken()->getText() << endl;
		...
		tokenizer->nextToken();
		tokenizer->lastToken()->youAreAFunction();
		Token * ident = tokenizer->lastToken();
		cout << "Function: " << ident->getText() << endl;
		...
		if (tokenizer->check(TT_COLON)) {
			...
			if (t) {
				if (tokenizer->peek(TT_IDENTIFIER)) {
					auto dl = parseDeclarationList();
					if (dl) {
						auto f = own(new Func(pos, ident, t->type, move(dl)));
						ident->setDeclaration(f.get());
						return f;
					}
					else
						return nullptr;
					...
					auto f = own(new Func(pos, ident, t->type, own(new DeclList)));
					ident->setDeclaration(f.get());
					return f;
				}
			}
...
unique_ptr<Func> Parser::parseFunctionBody(unique_ptr<Func> f) {
	//TODO - add code to save copy of token, set the function TT
	LOG << "Step Function Body" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
	...
	if (tokenizer->check(TT_RETURN)) {
		...
		if (!e) {
			...
			f->define(move(dl), move(sl), move(e));
			return f;
		}
	}
...
//Probably DONE!
unique_ptr<DeclList> Parser::parseDeclarationList() {
	LOG << "Step Declaration List" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
	auto vd = parseVariableDeclaration();
	if (!vd)
		return nullptr;
	//LOG << "Back to D list" << endl;
	if (tokenizer->check(TT_SEMICOLON)) {
		auto dl = parseDeclarationList();
		if (!dl) {
			LOG << "No data\n";
	...
	auto dl = own(new DeclList());
	dl->push_front(move(vd));
	//LOG << "End of D list" << endl;
	return dl;
}

unique_ptr<VarDecl> Parser::parseVariableDeclaration() {
	//TODO - add code to save copy of token, set the variable TT
	LOG << "Step Variable Declaration" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
	...
	tokenizer->nextToken();
	tokenizer->lastToken()->youAreAVariable();
	Token * ident = tokenizer->lastToken();
	...
	if (!t)
		return nullptr;
	//ident->youAreAVariable();
	auto vd = own (new VarDecl(pos,ident,move(t)));
	//LOG << "Done with VarDecl" << endl;
	return vd;
}

unique_ptr<Type> Parser::parseType() {
	LOG << "Step Scalar Type" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
...
unique_ptr<Expr> Parser::parseExpression() {
	//TODO - optimize
	LOG << "Step Expression" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;

	auto pos = tokenizer->currentPosition();
	if (tokenizer->check(TT_PLUS) || tokenizer->check(TT_MINUS) || tokenizer->check(TT_MULTIPLY) || tokenizer->check(TT_DIVIDE) || tokenizer->check(TT_MOD) || tokenizer->check(TT_LESS_THAN) || tokenizer->check(TT_GREATER_THAN) || tokenizer->check(TT_GREATER_THAN_OR_EQUAL) || tokenizer->check(TT_LESS_THAN_OR_EQUAL) || tokenizer->check(TT_EQUAL) || tokenizer->check(TT_NOT_EQUAL) || tokenizer->check(TT_AND) || tokenizer->check(TT_OR)) {
		auto op = tokenizer->lastToken()->getType();
		auto leftExpr = parseExpression();
			LOG << "Got left expression" << endl;
	
		auto rightExpr = parseExpression();
			LOG << "Got right expression" << endl;
	
		if (!leftExpr || !rightExpr) {
			//logPos();
			//ERROR << "Invalid syntax for binary expression: " << getToke() << endl;
			return nullptr;
		}
		return own(new BinaryExpr(pos, op, move(leftExpr), move(rightExpr)));
	}
	if (tokenizer->check(TT_NEG) || tokenizer->check(TT_NOT)) {
		...
		return own(new UnaryExpr(pos, op, move(expr)));
	}
	if (tokenizer->check(TT_CONSTANT) || tokenizer->check(TT_TRUE) || tokenizer->check(TT_FALSE)) {
		ScalarType s;
		if (tokenizer->lastToken()->getType() == TT_TRUE) {
			s = ST_BOOL;
			return own(new Constant(pos,true,s));
		}
		if (tokenizer->lastToken()->getType() == TT_FALSE) {
			s = ST_BOOL;
			return own(new Constant(pos,false,s));
		}
		else {
			s = ST_LONG;
			return own(new Constant(pos,tokenizer->lastToken()->getValue(),s));
		}
	}
	if (tokenizer->check(TT_INC) || tokenizer->check(TT_DEC)) {
		TokenType op = tokenizer->lastToken()->getType();
	...
	if (tokenizer->peek(TT_IDENTIFIER)) {
		//Check for Parameter list
			LOG << "It's an identifier, and: " << tokenizer->lastToken()->getText() << " " << tokenizer->gCurrentToken()->getText() << endl;
	
		if (tokenizer->gCurrentToken()->hasAttribute(TF_FUNCTION)) {
					LOG << "It's a function: " << tokenizer->gCurrentToken()->getText() << endl;
		
			auto id = tokenizer->lastToken();
					LOG << "Passed ident: " << id->getText() << endl;
			...
			return own(new FuncCall(pos,id,move(pl)));
		}
		if (tokenizer->gCurrentToken()->hasAttribute(TF_VARIABLE)) {
					LOG << "It's a variable, and: " << tokenizer->lastToken()->getText() << " " << tokenizer->gCurrentToken()->getText() << endl;
	...
	//logPos();
	//ERROR << "Expected operator, constant, identifier, or boolean value. Got: " << getToke() << endl;
	return nullptr;
}

unique_ptr<LValue> Parser::parseLValue() {
	LOG << "Step LValue" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
	...
	if (tokenizer->check(TT_IDENTIFIER)) {
		...
		auto ident = tokenizer->lastToken();
		if (tokenizer->check(TT_OPEN_ARRAY)) {
			auto expression = parseExpression();
			if (!expression) {
				logPos();
			...
			return own(new LValue(pos, ident, move(expression)));
		}
		return own(new LValue(pos, ident, nullptr));
	}
	logPos();
	ERROR << "Expected identifier or constant, got: " << getToke() << endl;
	return nullptr; 
...
unique_ptr<ExprList> Parser::parseParameterList() {
	...
	if (!tokenizer->check(TT_BANG)) {
		...
		pl->push_front(move(e));
		return pl;
	}
	auto pl = own(new ExprList);
	return pl;
}

unique_ptr<StmtList> Parser::parseStatementList() {

	LOG << "Step Statement List" << endl;
	LOG << tokenizer->gCurrentToken()->getText() << endl;
...
unique_ptr<Stmt> Parser::parseStatement() {
	...
	if (tokenizer->check(TT_SET)) {
		...
		return own(new AssignmentStmt(pos, move(lv), move(e)));
	}
	
	if (tokenizer->check(TT_OPEN_COMPOUND)) {
		auto msl = parseStatementList();
		if (msl && tokenizer->check(TT_CLOSE_COMPOUND)) {
			if (tokenizer->check(TT_CARET)) {
				auto e = parseExpression(); 
				if (!e) {
					logPos();
				...
				LOG << "Got '^' and expression." << endl;
				return own(new LoopStmt(pos, move(e), move(msl)));
			}
			...
			if (tokenizer->check(TT_IF)) {
				...
				if (tokenizer->check(TT_ELSE)) {
					bool isOpenCompound = tokenizer->check(TT_OPEN_COMPOUND);
					auto esl = parseStatementList();
					if (!(isOpenCompound && esl && tokenizer->check(TT_CLOSE_COMPOUND))) {
						logPos();
						ERROR << "Incorrect syntax for else clause." << endl;
						return nullptr;
					}
					return own(new ConditionalStmt(pos, move(e), move(msl), move(esl)));
				}
				else
					return own(new ConditionalStmt(pos, move(e), move(msl), move(own(new StmtList))));
			}
			logPos();
			ERROR << "Expected 'if', or '^', found: " << getToke() << endl;
		}
	...
	return own(new ExprStmt(pos, move(e)));
}
*/